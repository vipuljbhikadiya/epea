...
export default function edit({ attributes, setAttributes }) {
	const { taxonomy, blogsSlider, anchor } = attributes;

	// useSelect to retrieve all post types
	const taxonomies = useSelect(
		(select) =>
			select(coreStore).getEntityRecords("taxonomy", "blog_category", {
				per_page: -1,
			}),
		[]
	);
	...
	function onTaxCheckChange(newValue) {
		...
		data.sort();
		setAttributes({ taxonomy: [...data], blogsSlider: !blogsSlider });
	}
							...
							return (
								<>
									<CheckboxControl
										...
										name="taxonomy[]"
										checked={
											taxonomy ? taxonomy.indexOf(item.value) > -1 : false
										}
										onChange={(val) => {
