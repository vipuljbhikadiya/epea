...
register_post_meta('blog', 'seo_meta_title', [
	'show_in_rest' => true,
	'single' => true,
	'type' => 'string',
]);

register_post_meta('blog', 'seo_meta_description', [
	'show_in_rest' => true,
	'single' => true,
	'type' => 'string',
]);

register_post_meta('blog', 'seo_no_index', [
	'show_in_rest' => true,
	'single' => true,
	'type' => 'boolean',
]);

register_post_meta('blog', 'seo_no_follow', [
	'show_in_rest' => true,
	'single' => true,
	'type' => 'boolean',
]);
...
add_action('wp_head', 'dt_seo_tag_in_frontend');

if (!function_exists('dt_seo_tag_in_frontend')) {
	function dt_seo_tag_in_frontend()
	{
		$seo_meta_description = get_post_meta(get_the_ID(), 'seo_meta_description', true);

		if ('page' == get_post_type() || 'blog'  == get_post_type()) {
...
if (\function_exists('wp_robots')) {
	...
	add_filter('wp_robots', function ($robots) {
		...
		if ('page' == get_post_type() || 'blog'  == get_post_type()) {
			...
			if ($seo_no_index == true) {
				$robots['noindex'] = true;
			} else {
				$robots['noindex'] = false;
				$robots['index'] = true;
			}
			if ($seo_no_follow == true) {
