...
function sendEmail()
{
    ...
    if (isset($_REQUEST)) :
        ...
        if (!empty($_REQUEST['con_name']) || $timediff <= 3) {
            $response = array('status' => true, 'message' => __("Thank you for your message. It has been sent.", "epea-theme"));
        } else {
            ...
            if ('' != $_REQUEST['clientid']) :
                if (FALSE == get_option('cf-mail-' . $_REQUEST['clientid'])) {
                    $response = array('status' => false, 'message' => __('Something Went Wrong!', "epea-theme"));
                } else {
                    $sendMail = wp_mail(get_option('cf-mail-' . $_REQUEST['clientid']), get_option('cf-subject-' . $_REQUEST['clientid']), $message, $headers);
                    if (false == $sendMail) {
                        $response = array('status' => false, 'message' => __("There was an error trying to send your message. Please try again later.", "epea-theme"));
                    } else {
                        $response = array('status' => true, 'message' => __("Thank you for your message. It has been sent.", "epea-theme"));
                    }
                }
